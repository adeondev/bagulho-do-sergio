...
import { adicionarEvento, atualizarPainel, configurarPainel, setEstadoPausa, abrirPainel } from "./ui.js";
...
function update(time, delta) {
  ...
  fauna.update(d, habitantes, (animal, presa) => {
    ...
    if (Math.random() < 0.25) {
      eventoHabitante(presa, `${animal.def.nome} atacou ${presa.nome}!`);
    }
  });
...
function criarHabitantesNovos(scene, quantidade) {
  for (let i = 0; i < quantidade; i++) {
    ...
    const h = new Habitante(
      scene,
      NOMES[i % NOMES.length] + (i >= NOMES.length ? " II" : ""),
      ponto.x,
      ponto.y,
      tribo
    );
...
function configurarInterface() {
  configurarPainel({
    ...
    aoMudarVelocidade: v => {
      cena.simVelocidade = v;
    },

    aoMudarConcorrencia: v => {
      concorrencia = v;
    },

    aoSalvar: () => {
      salvarMundo(true);
      adicionarEvento(mundo, "Mundo salvo.");
    },
    ...
    aoPainelMudou: () => {
      setTimeout(() => {
        ...
        const painelAberto = document.body.classList.contains("painel-aberto");
        const larguraPainel = painelAberto ? painel.offsetWidth : 0;

        cena.scale.resize(window.innerWidth - larguraPainel, window.innerHeight);
      }, 50);
    }
  });

  document.getElementById("btn-dia")?.addEventListener("click", () => passarDia());
  document.getElementById("btn-mini-dia")?.addEventListener("click", () => passarDia());

  document.getElementById("btn-pause")?.addEventListener("click", alternarPausa);
  document.getElementById("btn-mini-pause")?.addEventListener("click", alternarPausa);
...
function atualizarCamera(delta) {
  ...
  if (esq || dir || cima || baixo) pararSeguir();
...
function contextoPainel() {
  return {
    ...
    estruturas,
    fauna,
    selecionado: habitanteSelecionado,
    pausado,
    era: estado.era,
    eraNome: nomeEra(estado.era),
    guerra: estado.guerra,
    tribos: resumoTribos(),
    concorrencia,
    velocidade: cena?.simVelocidade || 1
  };
}
...
function processarDecisoes(delta) {
  ...
  const ociosos = habitantes.filter(h =>
    ...
    !h.acaoPendente &&
    h.tempoConversa <= 0 &&
    h.tempoOcioso > h.proximoPasseioEm
  );
  ...
  executarEmLote(lote, concorrencia, async h => {
    ...
    .catch(e => console.warn("Decisoes:", e))
    .finally(() => {
      decisaoProcessando = false;
    });
}
...
function executarAcaoAutonoma(h, acao) {
  ...
  const alvo = acao.alvoHabitante
    ? habitantes.find(x => x.nome === acao.alvoHabitante && x.vivo)
    : null;
  ...
  switch (acao.tarefa) {
    ...
    case "comer": {
      ...
      if (fazenda) {
        ...
        if (q) {
          h.inventario.comida += q;
          h.fome = Math.max(0, h.fome - 20);
          return `colheu ${q} comida na fazenda`;
        }
      }
    ...
    case "construir": {
      ...
      if (!tipo || !ESTRUTURAS[tipo]) {
        return "tentou construir mas nao sabia o que";
      }
    ...
    case "craftar": {
      ...
      if (!id || !FERRAMENTAS[id]) {
        return "tentou fabricar algo desconhecido";
      }
      ...
      if (h.craftar(id)) {
        verificarEra();
        return `fabricou ${FERRAMENTAS[id].nome}`;
      }
    ...
    case "cacar": {
      ...
      if (!animal) {
        return "nao achou animal para cacar";
      }
...
function resolverComAlvo(h, alvo, acao) {
  ...
  if (hostil) {
    ...
    if (!alvo.vivo) {
      adicionarEvento(mundo, `${alvo.nome} foi derrotado por ${h.nome}.`);
    }
  ...
  if (h.tribo === alvo.tribo) {
    h.ajustarRelacao(alvo.nome, 2);
  }
...
async function executarInteracaoProxima(a, b) {
  ...
  if (a.vivo && b.vivo && Math.random() < 0.7) {
    ...
    if (acaoB) {
      rB = executarInteracaoAutonoma(b, a, acaoB);
    }
  }
...
function executarInteracaoAutonoma(ator, alvo, acao) {
  ...
  if (acao.categoria === "hostil" || acao.tarefa === "atacar") {
    ...
    if (alvo.vivo && (alvo.coragem > 45 || alvo.temArma()) && Math.random() < 0.6) {
      alvo.atacar(ator, "defesa");
    }

    registrarTensaoTribo(ator, alvo);

    if (!alvo.vivo) {
      adicionarEvento(mundo, `${alvo.nome} caiu em combate.`);
    }
...
function somaInventarios(rec) {
  return habitantes
    .filter(h => h.vivo)
    .reduce((s, h) => s + (h.inventario[rec] || 0), 0);
}

function totalFerramentas() {
  return habitantes
    .filter(h => h.vivo)
    .reduce((s, h) => s + h.ferramentas.size, 0);
}
...
function resumoTribos() {
  return TRIBOS.map(t => {
    ...
    return {
      ...
      moralMedia: membros.length
        ? Math.round(membros.reduce((s, h) => s + h.moral, 0) / membros.length)
        : 0
    };
  });
}
